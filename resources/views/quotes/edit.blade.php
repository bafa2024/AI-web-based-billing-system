...
@section('title', 'Edit Quote #{{ $quote->quote_number ?? $quote->id }} - Invoice Management System')

@section('page-title', 'Edit Quote #{{ $quote->quote_number ?? $quote->id }}')

@section('content')
<div class="bg-white p-8 rounded-lg shadow-sm">
    <!-- Header -->
    <div class="flex justify-between items-center mb-6">
        <h2 class="text-3xl font-bold text-gray-800">Edit Quote #{{ $quote->quote_number ?? $quote->id }}</h2>
    ...
    <form action="/quotes/{{ $quote->id }}" method="POST" class="space-y-6" id="quoteForm">
        ...
        <div class="grid grid-cols-3 gap-6">
            ...
            <div>
                ...
                <input type="date" id="quote_date" name="quote_date" value="{{ old('quote_date', $quote->quote_date ? $quote->quote_date->format('Y-m-d') : date('Y-m-d')) }}" required class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-red-500 focus:border-transparent @error('quote_date') border-red-500 @enderror">
                @error('quote_date')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
                                ...
                                <td class="px-6 py-4">
                                    <div class="space-y-2">
                                        <input type="text" 
                                               ...
                                               autocomplete="off">
                                        <input type="hidden" name="products[]" class="product-id-input" value="{{ $item->product_id }}">
                                        ...
                                        <textarea name="descriptions[]" 
                                                  ...
                                                  rows="2" 
                                                  placeholder="Product description (editable)">{{ old('descriptions.' . $index, $item->description) }}</textarea>
                                    </div>
                                </td>
                                <td class="px-6 py-4">
                                    <input type="number" name="quantities[]" value="{{ old('quantities.' . $index, $item->quantity) }}" min="1" class="quantity-input w-20 px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-red-500" required>
                                </td>
                                <td class="px-6 py-4">
                                    <input type="number" name="unit_prices[]" step="0.01" min="0" value="{{ old('unit_prices.' . $index, $item->unit_price) }}" class="unit-price-input w-24 px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-red-500" required>
                                </td>
                                <td class="px-6 py-4">
                                    <span class="line-total text-sm font-semibold text-gray-900">${{ number_format($item->total_price, 2) }}</span>
                                </td>
                                <td class="px-6 py-4">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
            <div>
                ...
                <div>
                    ...
                    <textarea id="notes" name="notes" rows="4" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-red-500 focus:border-transparent" placeholder="Any additional information...">{{ old('notes', $quote->notes) }}</textarea>
                </div>
            </div>
            ...
            <div class="bg-gray-50 p-6 rounded-lg">
                ...
                <div class="space-y-3">
                    <div class="flex justify-between items-center">
                        ...
                        <span id="subtotal" class="text-sm font-medium text-gray-900">${{ number_format($quote->subtotal, 2) }}</span>
                    </div>
                    
                    <div class="flex justify-between items-center">
                        <span class="text-sm text-gray-600">GST (5%):</span>
                        <span id="gst" class="text-sm font-medium text-gray-900">${{ number_format($quote->gst_amount, 2) }}</span>
                    </div>
                    
                    <div class="border-t border-gray-200 pt-3">
                        <div class="flex justify-between items-center">
                            <span class="text-lg font-semibold text-gray-900">Total:</span>
                            <span id="total" class="text-lg font-bold text-red-600">${{ number_format($quote->total_amount, 2) }}</span>
                        </div>
                    </div>
                </div>
            </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Product autocomplete (same as create page)
